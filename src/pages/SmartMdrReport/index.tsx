...
function NonActivityFeesPackagesListing() {
  ...
  const buildReportRequestString = async (fileName: string) => {
    var reportObject = '{"reportName":"smartMdrReport.jrxml"';
      ...
      ReportsService.getPDFReports(decodeURI(reportObject as string))
        .then(async (response) => {
          if (response.status === StatusCode.Success) {
            ...
            a.href = url;
            a.download = 'Smart MDR Report.pdf';
      ...
      ReportsService.getPDFReports(decodeURI(reportObject))
        .then(async (response) => {
          if (response.status === StatusCode.Success) {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = 'Smart MDR Report.xlsx';
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
          }
    ...
    <div className="wrapper">
      <main className="main-content">
        <div className="main-card">
          <div className="title-block">
            <div className="left-block">
              <Typography variant="h2">
                {intl.formatMessage({
                  id: "Reports.smartMdrReport",
                  defaultMessage: "Smart MDR Report",
                })}
              </Typography>
            </div>
          </div>
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} lg={4} sm={6} xl={4}>
              <div className="input-with-label form-group">
                <label>
                  {intl.formatMessage({
                    id: "Reports.smartMdrReport.fieldName",
          ...
          <Grid container mx={1} spacing={3} display="flex">
            <FormGroup row sx={{ mt: 2 }}>
              <FormControlLabel
                control={
                  <Checkbox
                    checked={groupWithMerchant}
                    onChange={(e) => setGroupWithMerchant(e.target.checked)}
                    name="groupWithMerchant"
                  />
                }
                label="Group with Merchant"
              />
              <FormControlLabel
                control={
                  <Checkbox
                    checked={groupWithTerminal}
                    onChange={(e) => setGroupWithTerminal(e.target.checked)}
                    name="groupWithTerminal"
                  />
                }
                label="Group with Terminal"
              />
            </FormGroup>
          </Grid>
