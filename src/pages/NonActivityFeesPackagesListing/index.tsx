...
function NonActivityFeesPackagesListing() {
  ...
  const getNonActivityByInstitutionId = async (id: string) => {
    await NonActivityFeesPackagesService.getNonActivityByInstitutionId(
      ...
      .then((res) => {
        setNonActivityFeesPackageList([...res.data]);
      })
  ...
  React.useEffect(() => {
    const setInstitutefromLocalStorage = async () => {
      ...
      if (instID) {
        ...
        getNonActivityByInstitutionId(instID);
  ...
  const handleChange = (event: SelectChangeEvent) => {
    ...
    getNonActivityByInstitutionId(selectedInstitutionId);
  };
  ...
  const changeStatus = async (id: string, event: any) => {
    ...
    NonActivityFeesPackagesService.changeStatus(model)
      .then((res) => {
        getNonActivityByInstitutionId(selectInstitutionVal);
  ...
  const editHandler = async (id: string) => {
    navigate(`/non-activity-fees-packages-definition/${id}`, {
  ...
  const onDelete = (id: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        NonActivityFeesPackagesService.deleteNonActivityFeePackage(id,selectInstitutionVal)
          .then((res) => {
  ...
  const getAllMappedEntities = async (id: string, unAssignedEntities: EntityListModel[]) => {
    ...
    await NonActivityFeesPackagesService.getAllMappedEntities(id).then(res => {
...
const getEntitiesBySearchCriteria = async () => {
  ...
  const mapPackageWithEntity = () => {
    ...
    NonActivityFeesPackagesService.mapPackageWithEntity(model).then(res => {
      if (res.status === StatusCode.Success) {
            ...
            <div className="title-block">
              <div className="left-block">
                <Typography variant={"h2"}>
                  {intl.formatMessage({
                    id: "NonActivityFeesPackage.subTitle",
                    defaultMessage: "Non Activity Fees Packages",
                  })}
                </Typography>
                <p className="pb-0">
                  {intl.formatMessage({
                    id: "NonActivityFeesPackage.list",
                    defaultMessage:
                      "List of defined Non Activity Fees Packages",
                  })}
                </p>
              </div>
              <div className="right-block">
                <Button
                  ...
                  onClick={() =>
                    navigate("/non-activity-fees-packages-definition", {
                      state: { institutionId: selectInstitutionVal },
                    })
                  }
                  disabled={!canAdd}
            ...
            <div className="form-group">
              <TableContainer>
                <Table sx={{ minWidth: 650 }} aria-label="simple table">
                    ...
                    <TableRow>
                      <TableCell width="30%">
                        ...
                        >
                          {
                            intl.formatMessage({
                              id: "NonActivityFeesPackageDefinition.packageId",
                  ...
                  <TableBody>
                    {NonActivityFeesPackageList.sort(getComparator(order, orderBy)).map((row, i) => (
                      <TableRow key={i}>
                    ...
                    {NonActivityFeesPackageList &&
                      NonActivityFeesPackageList.length === 0 && (
                        <TableRow>
          ...
          <DialogTitle component={"div"}>
            <div className="title-block mb-0">
              <div className="left-block mb-0">
                <Typography variant={"h2"}>
                  {intl.formatMessage({
                    id: "NonActivityFeesPackage.assignUnAssign.title",
            ...
            <div style={{ marginBottom: '20px', marginTop: '20px'}} className="btns-block right">
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={5}>
                  <div className="title">
                    {intl.formatMessage({
                      id: "NonActivityFeesPackage.assignUnAssign.allEntities",
                    ...
                    <FormGroup row className="checbox-grp">
                      <FormControlLabel
                        ...
                        label={intl.formatMessage({
                          id: "NonActivityFeesPackage.assignUnAssign.selectAll",
                ...
                <Grid item xs={12} md={5}>
                  <div className="title">
                    {intl.formatMessage({
                      id: "NonActivityFeesPackage.assignUnAssign.assignedEntities",
